...
async function callGraphQL(shortcode, docId, url) {
    if (process.env.INSTAGRAM_WORKER_URL) {
        ...
        console.log(`Using Cloudflare Worker for Instagram GraphQL: ${host}`);
        ...
        try {
            ...
            if (data && data.error) {
                console.error(`Worker error response: ${data.error}`);
...
async function resolveInstagramGraphQL(url) {
    ...
    try {
        ...
        if (response && (
            (response.data && (response.data.xdt_shortcode_media || response.data.shortcode_media)) ||
            (response.formats)
        )) {
            success = true;
        }
