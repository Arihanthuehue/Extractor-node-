...
async function getSession(url) {
  ...
  if (!session.cookies || isExpired) {
    try { await bootstrapSession(url); } catch (err) { return { cookies: null, csrftoken: null }; }
  }
  return session;
}
...
async function queryGraphQL(shortcode, docId, url) {
    ...
    if (process.env.INSTAGRAM_PROXY_URL) {
        args.push('--proxy', process.env.INSTAGRAM_PROXY_URL);
    }
    args.push("https://www.instagram.com/api/graphql");
